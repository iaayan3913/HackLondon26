...
function App() {
  ...
  const routeActiveNav = location.state?.activeNav
  const initialActiveNav = typeof routeActiveNav === 'string' && ROOT_NAV_ITEMS.has(routeActiveNav) ? routeActiveNav : 'dashboard'
    ...
    <div className="dashboard">
      ...
      <aside className="sidebar">
        ...
        <nav className="nav-menu">
          ...
          <button
            className={`nav-item ${activeNav === 'quizzes' ? 'active' : ''}`}
      ...
      <div className="main-container">
        ...
        <main className="content-area">
                ...
                <div className="hub-toolbar">
                  ...
                  <div className="hub-toolbar-actions">
                    <div className="view-toggle">
                      <button className={`view-toggle-button ${viewMode === 'grid' ? 'active' : ''}`} onClick={() => setViewMode('grid')}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" />
                          <rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" />
                        </svg>
                      </button>
                      <button className={`view-toggle-button ${viewMode === 'list' ? 'active' : ''}`} onClick={() => setViewMode('list')}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      ...
                      {files.map(file => (
                        <div key={file.id} className="hub-item file-item">
                          ...
                          <div className="hub-item-body">
                            ...
                            <div className="hub-item-meta">
                              <span className="file-badge" style={{ backgroundColor: file.color + '20', color: file.color }}>{file.type}</span>
                              <span>{file.size}</span>
                            </div>
                      ...
                      <tbody>
                        {folders.map(folder => (
                          <tr key={folder.id} className="hub-table-row folder-row">
                            ...
                            <td><div className="table-cell-content"><div className="folder-icon-small"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" /></svg></div><span>{folder.name}</span></div></td>
                            <td>Folder</td><td>{folder.items} items</td><td>{folder.modified}</td>
                            <td><button className="table-action-button"><svg viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="1.5" /><circle cx="12" cy="12" r="1.5" /><circle cx="12" cy="19" r="1.5" /></svg></button></td>
                          </tr>
                        ))}
                        {files.map(file => (
                          <tr key={file.id} className="hub-table-row file-row">
                            <td><input type="checkbox" /></td>
                            <td><div className="table-cell-content"><div className="file-icon-small" style={{ backgroundColor: file.color }}>{file.type}</div><span>{file.name}</span></div></td>
                            <td><span className="file-badge-small" style={{ backgroundColor: file.color + '20', color: file.color }}>{file.type}</span></td>
          ...
          {/* ───── OTHER PAGES (placeholders for teammates) ───── */}
          {activeNav === 'quizzes' && (
